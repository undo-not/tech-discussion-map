import type { AnalysisDelta, AnalysisKind, AnalysisOperation, AnalysisState, AnalysisStatus } from '../../domain/analysis/contract.ts';
...
export function analyzeWithDeterministicMock(utterances: TranscriptUtterance[], state: AnalysisState): AnalysisDelta {
  ...
  for (const utterance of finals) {
    ...
    if (matching) {
      if (!matching.evidenceUtteranceIds.includes(utterance.id)) operations.push({ op: 'update', itemId: matching.id, title: null, detail: null, status: null, confidence: Math.max(matching.confidence, 0.72), addEvidenceUtteranceIds: [utterance.id], removeEvidenceUtteranceIds: [] });
      continue;
    }
    ...
    operations.push({ op: 'add', tempId: `u_${utterance.id}`, kind: classified.kind, title: titleFor(text), detail: text.slice(0, 500), status: classified.status, confidence: 0.72, evidenceUtteranceIds: [utterance.id] });
  }
  return {
    contractVersion: 1,
