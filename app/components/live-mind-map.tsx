    ...
    <section aria-label="ライブ・ディスカッションマップ" className={`relative flex flex-col overflow-hidden rounded-2xl border border-[#d2d9d4] bg-[#e9eee9] shadow-[0_8px_30px_rgba(35,54,49,0.07)] xl:min-h-0 ${presentationMode ? 'min-h-[540px]' : 'min-h-[620px]'}`}>
      ...
      {selected && <aside aria-label="選択nodeの詳細" aria-live="polite" className="z-20 border-t border-[#d2dad4] bg-white/95 p-3 text-xs">
        {editing ? <div onKeyDown={(event) => { if (event.key === 'Escape') { event.preventDefault(); const focusId = editingItemId; setEditError(''); setEditing(false); setEditingItemId(''); requestAnimationFrame(() => focusNode(focusId, true)); } }} className="grid gap-2 md:grid-cols-[1fr_2fr_auto]">
          <p className="flex flex-wrap gap-2 text-[#52615c] md:col-span-3"><span>種類 {kindLabels[selected.kind]}</span><span>状態 {selected.status}</span><span>出所 {provenanceLabel[selected.provenance]}</span><span>根拠 {selected.evidenceUtteranceIds.join(' · ')}</span></p>
