...
The default analyzer is deterministic local mock. The browser UI owns the participant-consent, API-project data-control attestation, explicit external-analysis permission, and Credential Manager key-status gates; its authenticated request factory will not invoke OpenAI unless all are present. The loopback bearer authenticates the local UI but is not itself a consent record. Timeout, refusal, incomplete status, invalid JSON, invalid schema, stale revision, and upstream failure leave the local state unchanged. There is no automatic OpenAI retry. A 401 from the loopback companion causes one local bearer re-bootstrap and one replay to the companion; the rejected request never reached OpenAI. A single local apply queue serializes live model output with local state revisions.
