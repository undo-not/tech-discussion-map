...
Persisted retention choices are 1, 7, 30, or 90 days. Expired sessions are swept on privacy-store initialization and explicit refresh. Immediate delete first stops active input, drains pending encrypted writes, removes the entire encrypted session, and prevents that session ID from being persisted again. An unreadable session remains visible as unreadable and can be deleted without decryption. An unpersisted session never reaches disk. Local deletion does not delete a separately exported file or an external API copy. The deletion completion message states this unconditionally and never infers that no external copy exists. On browser startup, the obsolete `techmap-live-local` IndexedDB from pre-DPAPI builds is deleted without reading its plaintext.
...
Human-edited map titles and details remain local by default, but become part of that bounded state projection on a later OpenAI analysis while the external-analysis gate is enabled. The editor warns that heuristic redaction does not reliably remove personal or organization names; the actual combined outbound preview remains the final authority before sending.
