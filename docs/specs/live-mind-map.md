...
Pure domain tests cover stable placement with 100 nodes, deterministic allocation for new nodes, bounded unified undo/redo, forward-only revision behavior, rejection of an AI overwrite after human confirmation, suppression of duplicate mock nodes after confirmation, selection replacement, edit-item binding, the scroll target for distant keyboard navigation, a mixed 300-node degraded viewport with multi-node top alignment and one-shot filter guard, and fresh slots for a loaded session. The app gate also runs typecheck, lint, production build, privacy checks, and the existing native Windows CI jobs.
